...
import Pending from './productdetails/Pending';
import Delivered from './productdetails/Delivered';
import Shipped from './productdetails/Shipped';
import ReturnRequest from './productdetails/ReturnRequest';
import AllOrders from './productdetails/AllOrders';
...
const ProductsDashboard = () => {
  ...
  return (
    <div className="p-4 space-y-6 min-h-screen" style={{
      overflowY: "auto",
      scrollbarWidth: "none",
      ...
      {/* Only show these sections when not showing detail view */}
      {!showDetail && (
        <>
          {/* Top Stats */}
          <div className="flex flex-row justify-start bg-bluebutton p-4 rounded-md gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl p-4 shadow">
              ...
              <ResponsiveContainer width="100%" height={150}>
                <BarChart data={barData}>
                  ...
                  <YAxis hide />
                  <Tooltip />
                  <Bar dataKey="value" fill="#0b5ed7" radius={[2, 2, 0, 0]} barSize={20} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow">
              ...
              <ResponsiveContainer width="100%" height={150}>
                <LineChart data={lineData}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="value" stroke="#8f00ff" strokeWidth={3} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white rounded-xl p-4 shadow">
                ...
                <p className="text-2xl font-bold">56 <span className="text-green-500 text-sm">▲ 6.65%</span></p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow">
                <p className="font-medium">Recent Delivered</p>
                <p className="text-2xl font-bold">125 <span className="text-green-500 text-sm">▲ 6.65%</span></p>
              </div>
            </div>
          </div>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-full ${
                  activeTab === tab.id ? 'text-bluebutton' : 'text-graycolor opacity-[0.66]'
                }`}
              >
                {tab.name}
                {tab.id === 'ReturnRequest' && (
                  <span className="ml-2 bg-bluebutton text-white rounded-full px-2 py-0.5 text-xs">20</span>
                )}
              </button>
            ))}
          </div>
