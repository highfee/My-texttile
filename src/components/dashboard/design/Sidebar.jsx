...
const Sidebar = () => {
  ...
  // Handle adding a template to the canvas
  ...
  const handleAddShape = (shape) => {
    const shapeProperties = {
      shapeType: shape.id,
      fill: selectedProductColor || "#000000",
      x: 100,
      y: 100,
      width: 100,
      height: 100,
    ...
    addElement("shape", shapeProperties);
  };

  // Handle adding an image to the canvas
  const handleAddImage = () => {
    // This would typically open a file dialog or media library
    // For now we'll just add a placeholder image
    const imageProperties = {
      src: "/design/images/upload 1.png",
      x: 100,
      y: 100,
      width: 150,
      height: 150,
    ...
    addElement("image", imageProperties);
  };
  ...
  const handleAddText = () => {
    ...
    addElement("text", textProperties);
  };

  // Handle product color selection
  const handleColorChange = (colorId, colorHex) => {
    setSelectedColor(colorId);
    setProductColor(colorHex);
  };

  // Handle brand kit selection
  const handleBrandSelect = (kitId) => {
    setActiveBrandKit(kitId);
  };

  // Handle adding a logo from a brand kit
  const handleAddLogo = (logo) => {
    ...
    addElement("image", logoProperties);
  };

  // Handle adding an upload to the canvas
  const handleAddUpload = (upload) => {
    ...
    addElement("image", uploadProperties);
  };
  ...
  const filteredTemplates = React.useMemo(() => {
    ...
    return {
      recent: recentlyUsed.filter(filterByName),
      premium: premiumTemplates.filter(filterByName),
      free: freeTemplates.filter(filterByName),
    };
  }, [searchTerm]);
      ...
      <Tabs defaultValue="styles" className="w-full">
        ...
        <TabsContent value="styles">
          <header className="flex justify-between gap-5 items-center mt-5">
            <Button variant="outline" className="h-12" onClick={handleAddImage}>
              <Image
                src={"/design/icons/image.svg"}
                alt=""
                height={26}
            ...
            <Button variant="outline" className="h-12" onClick={handleAddText}>
              <Image
                src={"/design/icons/text-creation.svg"}
                alt=""
                height={26}
