...
const Createaccount = ({ onBack }) => {
  const [selectedCountry, setSelectedCountry] = useState("US");// State for selected country
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const countries = [
    { code: "US", name: "United States", flag: <US className=" w-5" /> },
    { code: "IN", name: "India", flag: <IN className=" w-5" /> },
    { code: "GB", name: "United Kingdom", flag: <GB className=" w-5" /> },
    { code: "CA", name: "Canada", flag: <CA className=" w-5" /> },
    { code: "AU", name: "Australia", flag: <AU className=" w-5" /> },
    { code: "FR", name: "France", flag: <FR className=" w-5" /> },
    { code: "DE", name: "Germany", flag: <DE className=" w-5" /> },
    { code: "JP", name: "Japan", flag: <JP className=" w-5" /> },
  ];
  ...
  return (
    <div className="flex flex-row items-stretch  rounded-lg shadow-md overflow-hidden sm:w-[600px] md:w-[700px] lg:w-[850px] h-auto sm:h-[400px] md:h-[450px] lg:h-[772px] mx-auto">
      <div className="flex flex-col items-start px-2 sm:px-6 py-6 sm:py-8 md:py-10 w-full md:w-1/2">
        <div>
          <FaArrowLeftLong
            className="text-black cursor-pointer"
            onClick={onBack} // Go back to Continuewithemail
          />
        </div>
        ...
        <div className="flex flex-col gap-4 w-full">
          <div className="w-full ">
            ...
            <div className="flex items-center w-full border border-[#12121270] rounded-lg focus:outline-none">
      {/* Flag Icon with Dropdown */}
      ...
      />
          ...
          <div className="w-full py-2">
            <button onClick={handleContinue} className="w-full bg-bluebutton text-white py-2 rounded-lg">
              <p>Continue</p>
            </button>
          </div>
        </div>
